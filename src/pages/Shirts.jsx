  ...
  useEffect(() => {
    const fetchProducts = async () => {
      try {
        ...
        // Get shirts from both the shirts array and filtered products array
        const shirtsFromProducts = (data.products || []).filter(
          product => product.category && product.category.toLowerCase() === 'shirts'
        );
        
        // Combine both sources of shirts data
        const allShirts = [...(data.shirts || []), ...shirtsFromProducts];
        
        setProducts(allShirts);
        setLoading(false);
      } catch (error) {
            ...
            <div className="products-grid">
              {products.map(product => (
                <div 
                  ...
                  onClick={() => navigate(`/shirts/${product.id}`)}
                >
